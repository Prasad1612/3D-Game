...
class GameUI:
    ...
    def update_radar(self, enemies):
        ...
        for e in enemies:
            ...
            dist = rel_pos.length()  # Use length() instead of distance to Vec3(0,0,0)
            if dist > radar_range:
                rel_pos = rel_pos.normalized() * radar_range
...
class Enemy(Entity):
    ...
    def update(self):
        ...
        if self.intersects(player).hit:  # Use intersects for consistency
            ui.health -= 1
            destroy(self)
            if self in enemies:
                enemies.remove(self)
...
class Bullet(Entity):
    ...
    def update(self):
        ...
        for e in enemies[:]:  # Copy list to avoid modification issues
...
# === Game Over Handling ===
def game_over():
    global game_over_active, game_over_text
...
def update():
    global spawn_timer
    if game_over_active:
        return  # Skip updates during game over
...
def load_sound(file):
    try:
        ...
        return Audio(None, autoplay=False)  # Silent fallback
